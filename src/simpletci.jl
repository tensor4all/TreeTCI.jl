...
function sweep2site!(
    ...
    for iter = iter1:iter1+niter-1
        ...
        while true
            ...
            candidates = vcat(
                [(c, q_, p_) for c in candidate_bondids(tci.g, p_, q_, tci.idbonds)],
            ...
            incomings =
                candidate_bondids(tci.g, center_info[2], center_info[3], tci.idbonds)
            ...
            updatepivots!(
                ...
                subIJkey = subIJkey,
                extraIJset = Dict(
                    first(IJkeys) => extraIJset[first(IJkeys)],
                    last(IJkeys) => extraIJset[last(IJkeys)],
                ),
            )
...
function fillsitetensors!(tci::SimpleTCI{ValueType}, f) where {ValueType}
    for b = 1:length(tci)
...
function setsitetensor!(
    ...
    J_key = collect(b+1:L)
    tci.sitetensors[b] =
        reshape(T, length(tci.Iset[I_key]), tci.localdims[b], length(tci.Jset[J_key]))
end
...
function updatepivots!(
    ...
    extraIJset::Dict{Vector{Int},Vector{MultiIndex}} = Dict{
        Vector{Int},
        Vector{MultiIndex},
    }(),
) where {F,ValueType}
    ...
    Iset =
        kronecker(tci.IJset, first(IJkeys), first(subIJkey), tci.localdims[first(subIJkey)])
    ...
    luci = if pivotsearch === :full
        t1 = time_ns()
        Pi = reshape(
            filltensor(ValueType, f, tci.localdims, Icombined, Jcombined, Val(0)),
            length(Icombined),
            length(Jcombined),
        )
        t2 = time_ns()
    ...
    elseif pivotsearch === :rook
        ...
        I0 = Int.(
            Iterators.filter(
                !isnothing,
                findfirst(isequal(i), Icombined) for i in tci.Iset[b+1]
            )
        )::Vector{Int}
        J0 = Int.(
            Iterators.filter(
                !isnothing,
                findfirst(isequal(j), Jcombined) for j in tci.Jset[b]
            )
        )::Vector{Int}
        ...
        if verbosity > 2
            x, y = length(Icombined), length(Jcombined)
            println(
                "    Computing Pi ($x x $y) at bond $b: $(1e-9*(t2-t1)) sec, LU: $(1e-9*(t3-t2)) sec, fall back to full: $(1e-9*(t4-t3)) sec",
            )
        end
        res
    else
        throw(
            ArgumentError(
                "Unknown pivot search strategy $pivotsearch. Choose from :rook, :full.",
            ),
        )
    end
    I_key = collect(1:b)
...
function setsitetensor!(
    ...
    if (leftorthogonal && b == length(tci)) || (!leftorthogonal && b == 1)
...
function updateerrors!(tci::SimpleTCI{T}, b::Int, errors::AbstractVector{Float64}) where {T}
    updatebonderror!(tci, b, last(errors))
    updatepivoterror!(tci, errors)
    nothing
end

function updatebonderror!(tci::SimpleTCI{T}, b::Int, error::Float64) where {T}
...
function filltensor(
    ...
    return reshape(_call(ValueType, f, localdims, Iset, Jset, Val(ncent)), expected_size...)
end
...
function kronecker(
    ...
    return MultiIndex[
        [is[1:site_index-1]..., j, is[site_index:end]...] for is in pivotset, j = 1:localdim
    ][:]
end

function kronecker(Iset::Union{Vector{MultiIndex},TCI.IndexSet{MultiIndex}}, localdim::Int)
    return MultiIndex[[is..., j] for is in Iset, j = 1:localdim][:]
end

function kronecker(localdim::Int, Jset::Union{Vector{MultiIndex},TCI.IndexSet{MultiIndex}})
    return MultiIndex[[i, js...] for i = 1:localdim, js in Jset][:]
end
...
function _call(
    ...
    result = Array{V,3}(
        undef,
        length(leftindexset),
        prod(localdims[nl+1:L-nr]),
        length(rightindexset),
    )
    for (i, lindex) in enumerate(leftindexset)
        for (c, cindex) in
            enumerate(Iterators.product(ntuple(x -> 1:localdims[nl+x], M)...))
